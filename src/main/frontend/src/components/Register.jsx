...
export default function Register() {
    ...
    const [pmSummary, setPmSummary] = useState(""); // p.ej. "Visa •••• 1234"
    ...
    const detectBrand = (num) => {
        ...
        if (/^4\d{12,18}$/.test(n)) return "Visa"; // Visa
    ...
    const isValidCvv = (cvv) => /^\d{3}$/.test(cvv); // Visa/MC => 3 dígitos
    ...
    const handleTelefonoChange = (e) => {
        setTelefono(onlyDigits(e.target.value).slice(0, 15)); // margen superior
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (!calle || !numero) {
            setError("Completá tu domicilio (calle y número).");
            return;
        }
