...
import { authService } from "../services/authService"; // Importar tu authService
...
export default function CartPage() {
    ...
    const loadAddress = async (userId) => {
        try {
            console.log('📍 Cargando dirección para userId:', userId);
            const token = authService.getToken();

            const response = await axios.get(`http://localhost:8080/api/cart/${userId}/address`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });

            console.log('✅ Dirección cargada:', response.data);
            ...
            // No es crítico, el usuario puede escribir la dirección manualmente
        }
    };
                                ...
                                <div className="mb-6">
                                    ...
                                    {clientAddress && (
                                        <p className="text-xs text-gray-500 mt-1">
                                            Dirección guardada: {clientAddress}
                                        </p>
                                    )}
                                </div>
